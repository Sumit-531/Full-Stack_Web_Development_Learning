...
// Create a TypeScript type from the Zod schema so the runtime validation 
// (EmployeeSchema) and static typing (Employee) always stay in sync.
type Employee = z.infer<typeof EmployeeSchema>;

// Create a StructuredOutputParser that knows how to validate and format
// an array of Employee objects using the Zod schema "EmployeeSchema".
// Using Zod to ensure type safety and a parser that will help to generate structured data from the AI output
...
async function generateSyntheticData(): Promise<Employee[]>{
    const prompt = `You are a helpful assistant that generates employee data. Generate 10
    fictional employee records. Each record should include the following fields: employee_id, 
    first_name, last_name, date_of_birth, address, contact_details, job_details, work_location, 
